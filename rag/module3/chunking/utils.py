...
def kill_processes_on_ports(
    ...
    *,
    ...
    try:
    ...
    except (psutil.AccessDenied, PermissionError, RuntimeError, Exception) as e:
        # macOS restricts psutil.net_connections without root.
        # Fall back to lsof to find PIDs on the target ports.
        ...
        for port in target_ports:
            ...
            except (_sp.CalledProcessError, ValueError):
                pass
        ...
        for pid in list(pids):
            try:
                ...
                results['errors'].append({'pid': pid, 'error': f'Access denied: {e2}'})
            except Exception as e2:
                results['errors'].append({'pid': pid, 'error': str(e2)})
        ...
        if alive and force:
            for p in alive:
                try:
                    ...
                    results['errors'].append({'pid': p.pid, 'error': f'Access denied on kill: {e2}'})
                except Exception as e2:
                    results['errors'].append({'pid': p.pid, 'error': str(e2)})
            gone2, _ = psutil.wait_procs(alive, timeout=timeout)
            for p in gone2:
                try:
...
def generate_with_single_input(prompt: str,
    ...
    try:
        ...
        raise Exception(f"Error while calling LLM: {e}")
